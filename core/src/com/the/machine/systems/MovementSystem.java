	...
	protected void processEntity(Entity entity, float deltaTime) {
        ...
        if (!draggedEntities.has(entity)) {
            ...
            if (velocityComponent.isDirty()) {
                ...
                physics2dComponent.getBody().setLinearVelocity(new Vector2(deltaPosition.x/10, deltaPosition.y/10));
                velocityComponent.setDirty(false);
            } else {
                velocityComponent.setVelocity(physics2dComponent.getBody().getLinearVelocity().len()*10);
            }
